...
@app.route("/api/superadmin/recovery", methods=["POST"])
def superadmin_recovery():
    """
    Recuperación de emergencia del único SUPERADMIN.
    Requiere SUPERADMIN_RECOVERY_SECRET configurado fuera del código.
    No devuelve ni registra secretos, contraseñas ni hashes.
    """
    conn = get_db()
    if rate_limit_exceeded(conn, "superadmin_recovery"):
        conn.close()
        return rate_limit_response()

    recovery_secret = os.environ.get("SUPERADMIN_RECOVERY_SECRET", "").strip()
    if not recovery_secret or len(recovery_secret) < 32:
        conn.close()
        return jsonify({"error": "La recuperación de emergencia no está habilitada."}), 503

    data = request.get_json(silent=True) or {}
    provided_secret = str(data.get("recovery_secret", "")).strip()
    nueva_password = str(data.get("nueva_password", ""))
    if not provided_secret or not secrets.compare_digest(
        provided_secret.encode("utf-8"), recovery_secret.encode("utf-8")
    ):
        conn.close()
        return jsonify({"error": "Credencial de recuperación inválida."}), 403
    ...
    try:
        ...
        usuario_configurado = os.environ.get("SUPERADMIN_USUARIO", "").strip()
        if not usuario_configurado or usuario_configurado.lower() in {
            "el usuario que quieras conservar/crear", "superadmin"
        }:
            usuario_configurado = "superadmin"
        correo_configurado = os.environ.get(
            "SUPERADMIN_EMAIL", "kakuaaconsultores@gmail.com"
        ).strip() or "kakuaaconsultores@gmail.com"

        if superadmin:
            usuario = superadmin["usuario"] or usuario_configurado
            if usuario in {"el usuario que quieras conservar/crear"}:
                usuario = usuario_configurado
            usuario_id = superadmin["id"]
            correo = superadmin["correo"] or correo_configurado
            conn.execute(
                """UPDATE usuarios
                   SET usuario = ?, correo = ?, password_hash = ?, activo = 1,
                       debe_cambiar = 0, intentos_fallidos = 0, bloqueo_hasta = NULL,
            ...
            ruc = "SUPERADMIN-000000"
            conflicto = conn.execute(
                "SELECT id FROM usuarios WHERE usuario = ? OR correo = ? OR ruc = ? LIMIT 1",
                (usuario, correo, ruc),
            ).fetchone()
            if conflicto:
                conn.rollback()
                conn.close()
                return jsonify({
                    "error": "No se puede crear el SUPERADMIN porque existe un conflicto de usuario, correo o RUC."
                }), 409
            cur = conn.execute(
        ...
        conn.execute(
            """UPDATE usuarios
               SET token_sesion = NULL, token_sesion_hash = NULL,
                   csrf_token_hash = NULL, token_expira_en = NULL,
                   reset_token_hash = NULL, reset_expira_en = NULL
               WHERE id = ?""",
            (usuario_id,),
        ...
        conn.close()
        return jsonify({
            "error": "No se pudo completar la recuperación por un conflicto de datos."
        }), 409
    except Exception:
        conn.rollback()
        conn.close()
        return jsonify({"error": "No se pudo completar la recuperación."}), 500
